...
public class CheckBundleDupeDependenciesMultiIsolatedGroups : CheckBundleDupeDependencies
{
    ...
    public override void FixIssues(AddressableAssetSettings settings)
    {
        ...
        Dictionary<UnityEngine.GUID, List<AddressableAssetGroup>> implicitAssetsToGroup = GetImplicitAssetsToGroup(CheckDupeResults);

        var groupsToAssets = new Dictionary<List<AddressableAssetGroup>, List<UnityEngine.GUID>>(new GroupComparator());
        foreach (KeyValuePair<UnityEngine.GUID, List<AddressableAssetGroup>> pair in implicitAssetsToGroup)
        {
            if (!groupsToAssets.TryGetValue(pair.Value, out List<UnityEngine.GUID> assets))
            {
        ...
        foreach (KeyValuePair<List<AddressableAssetGroup>, List<UnityEngine.GUID>> pair in groupsToAssets)
        {
            ...
            foreach (UnityEngine.GUID asset in pair.Value)
    ...
    protected Dictionary<UnityEngine.GUID, List<AddressableAssetGroup>> GetImplicitAssetsToGroup(IEnumerable<CheckDupeResult> checkDupeResults)
    {
        var implicitAssetsToGroup = new Dictionary<UnityEngine.GUID, List<AddressableAssetGroup>>();
        foreach (var checkDupeResult in checkDupeResults)
        {
            UnityEngine.GUID assetGuid = checkDupeResult.DuplicatedGroupGuid;
